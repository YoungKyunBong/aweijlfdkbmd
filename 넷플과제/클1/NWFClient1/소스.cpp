...
HINSTANCE g_hInst;
int retval;
LPCTSTR lpszClass = TEXT("Client 1");
...
{
	HDC hdc;
	static HBITMAP  ShadowAttackBitmap, GreenAttackBitmap, ShawdowStandingBitmap, GreenStandingBitmap, mainBitmap;
	PAINTSTRUCT ps;
	switch (iMessage)
	{
	case WM_KEYDOWN:
	...
	case WM_CREATE:
		...
		ShawdowStandingBitmap = LoadBitmap(g_hInst, MAKEINTRESOURCE(IDB_BITMAP6));
	...
	case WM_PAINT:
		...
		if (WinTheGame == TRUE)
		{
			TextOut(hdc, 350, 50, TEXT("1P WIN"), 6);
		}
		if (clientTwoMotionCount == 0)
		{
			DrawShadowStand(hdc, clientTwoXpos, clientTwoYpos, ShawdowStandingBitmap);
		}
		...
		if (clientOneMotionCount == 0)
		{
			...
			Sleep(200);
			clientOneMotionCount = 2;
		}
		else if (clientOneMotionCount == 2)
		{
			DrawGreenStand(hdc, clientOneXpos, clientOneYpos, GreenStandingBitmap);
		}
		EndPaint(hWnd, &ps);
...
DWORD WINAPI ClientMain(LPVOID arg)
{
	...
	while (1) {
		...
		if(strcmp(buf , "C1DG") == 0)
		{
			clientTwoMotionCount = 1;
			clientOneHp -= 1;
			printf("1P 남은 체력: %d \n", clientOneHp);
		}
		else if (strcmp(buf, "C2DG") == 0)
		{
			clientTwoHp -= 1;
